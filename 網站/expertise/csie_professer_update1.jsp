...
ResultSet rs = odbcbean.doSelect("select * from Expertise") ;
%>
<html>
<head>
...
<p align="center">
<p align="center">
<form name="form1" method="post" action="csie_professer_update1.jsp">
<select size="1" name="productid" onChange="go();">
	<option value="" selected >請選擇專長</option>
	<%while(rs!=null && rs.next()){%>
		<option value="<%=rs.getString("ExpertiseID").trim()%>"><%=rs.getString("ExpertiseName").trim()%></option>
	<%}%>
</select>
</form>


<form name="form2" method="post" action="csie_professer_update2.jsp">
...
<div class="container mt-2">
	...
	<%if(productid!=null && !productid.trim().equals("")){
		rs = odbcbean.doSelect("select * from Expertise WHERE ExpertiseID='" + productid + "'") ;
		String pkey = "" ;
		while(rs!=null && rs.next()){
			pkey = rs.getString(1);
	%>
  <tr>
  	...
  	<input type="hidden" name="expertiseid" value="<%=pkey%>">
    <td width="16%"><%=pkey%></td>
 
    <td width="17%" ><input type="text" name="expertisename" size="20" value="<%=rs.getString(2)%>" ></td>
